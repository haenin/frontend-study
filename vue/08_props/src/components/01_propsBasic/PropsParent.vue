<!-- 
    ...
    Prop는 기본적으로 readonly의 개념을 통해 자식 컴포넌트에서
    수정해도 부모에 영향을 주지 안는다. (안티패턴 방지)
...
<template>
    <div :class="{container:true, dark:darkState}">
        <!-- 
            ...
            -> Props는 일반적으로 같은 이름을 사용 
        -->
        <!-- 내부적으로 자식은 read only -->
        <PropsChild :message="message2"/>

        <!-- 
            Props로 물려준 부모의 상태는 자식에서 수정하더라도
            부모의 상태는 영향을 받지 않음(전혀 다른 객체)
        -->
        <ReadProps :readValue="readValue"/>
        <br>
        {{ readValue }}

        <!-- 함수를 물려준게 아니라 이벤트를 물려준 것 -->
